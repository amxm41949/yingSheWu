...
while (ROUNDS--) {
    ...
    for (let i = 0; i < gameState.snake_num; i++) {
        scores[i] += finalResults.scores[i];
        console.log(`Snake ${i + 1}: ${scores[i]} points${finalResults.alive[i] ? " (survived)" : " (died in round " + (finalResults.dead_round[i]) + ")"} spent ${finalResults.time[i].toFixed(3)}ms`);
    }
}
