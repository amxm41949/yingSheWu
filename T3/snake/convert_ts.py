...
# 加载 state_dict
state_dict = torch.load("./snake/sotas/dqn_model_8_sota.pth")
...
for key, value in state_dict.items():
    ...
    if weight_array.ndim == 1:
        # 1D 数组：f32[]
        ts_value = f'export const {ts_key}: f32[] = [{", ".join(map(str, weight_array.flatten()))}];\n'
    elif weight_array.ndim == 2:
        # 2D 数组：f32[][]
        ts_value = 'export const ' + ts_key + ': f32[][] = [\n'
...
with open("weights.ts", "w") as f:
    f.write(ts_code)

print("TypeScript 文件 'weights.ts' 已生成。")